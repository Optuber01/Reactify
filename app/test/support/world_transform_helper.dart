...
AffineMatrix resolveTestWorldTransform(
  ...
  final poseThighBack =
      tables
          .posePlacementFor(pose: state.numeric('pose'), hostName: 'thigh_back')
          ?.matrix ??
      const AffineMatrix.identity();
  final poseFeetFront =
      tables
          .posePlacementFor(pose: state.numeric('pose'), hostName: 'foot_front')
          ?.matrix ??
      const AffineMatrix.identity();
  final poseFeetBack =
      tables
          .posePlacementFor(pose: state.numeric('pose'), hostName: 'foot_back')
          ?.matrix ??
      const AffineMatrix.identity();

  final torsoLocalBase = poseTorso;
  final headLocalBase = poseTorso.inverse().multiply(poseHead);
  final shoulderFrontLocalBase = poseTorso.inverse().multiply(
    poseShoulderFront,
  );
  final shoulderBackLocalBase = poseTorso.inverse().multiply(poseShoulderBack);
  final forearmFrontLocalBase = poseShoulderFront.inverse().multiply(
    poseForearmFront,
  );
  final forearmBackLocalBase = poseShoulderBack.inverse().multiply(
    poseForearmBack,
  );
